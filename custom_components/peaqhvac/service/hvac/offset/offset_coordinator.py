...
class OffsetCoordinator:
    ...
    def _update_offset(self, weather_adjusted_today: dict | None = None) -> dict:
        ...
        cached_tomorrow = cache.get_cache_for_today(
            (datetime.now() + timedelta(days=1)).date(), self.prices_tomorrow
        )
        cached_midnight_problem = cache.get_cache_for_today(
            datetime.now().date() - timedelta(days=1), self.prices
        )
        ...
        try:
            ...
            all_values = set_offset_dict(
                self.prices + self.prices_tomorrow, datetime.now(), self.min_price
            )
            _LOGGER.debug(
                "all_values",
                all_values,
            ...
            offsets_per_day = self._calculate_offset_per_day(
                all_values, weather_adjusted_today
            )
    ...
    def _calculate_offset_per_day(
        self, day_values: dict, weather_adjusted_today: dict | None = None
    ) -> dict:
    ...
    def _set_offset(self) -> None:
        if self.prices is not None:
            ...
            if len(self._hub.prognosis.prognosis) > 0:
                try:
                    ...
                    if len(_weather_dict.items()) > 0:
                        _LOGGER.debug("weather-prognosis", _weather_dict)
                        self.model.calculated_offsets = self._update_offset(
                            _weather_dict
                        )
                except Exception as e:
                    _LOGGER.warning(
            ...
            if self._hub.is_initialized:
                _LOGGER.warning(
                    f"Hub is ready but I'm unable to set offset. Prices num:{len(self.prices) if self.prices else 0}"
                )
