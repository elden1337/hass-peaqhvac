...
class NextWaterBoost:
    ...
    @staticmethod
    def _calculate_target_temp_for_hour(
        temp_at_time: float,
        is_demand: bool,
        price: float,
        price_spread: float,
    ...
    def _get_temperature_at_datetime(
        now_dt, target_dt, current_temp, temp_trend
    ) -> float:
    ...
    def _add_data_list(self, model: NextStartPostModel) -> list:
        ...
        for idx, p in enumerate(model.prices[self.dt.hour :], start=self.dt.hour):
            new_hour = (self.dt + timedelta(hours=idx - self.dt.hour)).replace(
                minute=50, second=0, microsecond=0
            )
            second_hour = self.dt + timedelta(hours=idx - self.dt.hour + 1)
            temp_at_time = self._get_temperature_at_datetime(
                self.dt, new_hour, model.current_temp, model.temp_trend
            )
            ...
            data.append(
                PriceData(
                    ...
                    new_hour.hour in model.non_hours
                    or second_hour.hour in model.non_hours,
                    self._calculate_target_temp_for_hour(
                        temp_at_time,
                        second_hour.hour in model.demand_hours,
                        p,
                        round(p / mean(model.prices[idx - self.dt.hour :]), 2),
    ...
    def _calculate_is_cold(
        self,
        temp_at_time: float,
        second_hour: datetime,
        ...
        for d in data:
            if all(
                [
                    ...
                    (
                        d.price_spread < 1
                        or d.price < self.min_price
                        or (d.is_demand or d.water_temp < d.target_temp)
                    ),
                    not d.is_non,
    ...
    def get_filtered(self, data: list, selected: PriceData) -> list:
        return [
            d
            for d in data
            if max(d.time, selected.time) - min(d.time, selected.time)
            <= timedelta(hours=2)
            and d.time >= self.reset_hour(self.dt)
        ]
    ...
    def get_final_selected(self, filtered: list, selected: PriceData) -> PriceData:
        ...
        for d in sorted(filtered, key=lambda x: (not x.is_demand, x.price_spread)):
            if (
                not d.is_non
                and d.price_spread < selected.price_spread
                and not selected.is_demand
                and not selected.water_temp < self.low_water_limit
            ):
                selected = d
                break
        return selected
