...
class StateChanges:
    ...
    async def async_update_sensor(self, entity, value):
        if entity in self._hub.options.indoor_tempsensors:
            ...
            await self._hub.sensors.temp_trend_indoors.async_add_reading(val=self._hub.sensors.average_temp_indoors.value, t=time.time())
            ...
            await self._hub.sensors.temp_trend_outdoors.async_add_reading(val=self._hub.sensors.average_temp_outdoors.value, t=time.time())
        await self._hass.async_add_executor_job(self._hub.prognosis.get_hvac_prognosis,self._hub.sensors.average_temp_outdoors.value)
