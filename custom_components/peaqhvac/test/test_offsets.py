...
P231213 = [1.17, 1.14, 1.14, 1.11, 1.11, 1.14, 1.25, 1.59, 2.09, 2.09, 2.13, 2.14,2.14, 1.61, 1.59, 1.62, 1.61, 1.68, 1.61, 1.52, 1.44, 1.36, 1.38, 1.27]
...
@pytest.mark.asyncio
async def test_offsets_cent_and_normal_match():
    prices = P231213 + P231214
    now_dt = datetime(2023,12,13,20,43,0)
    r1 = await set_offset_dict(prices, now_dt, 0, {})
    r2 = await set_offset_dict([p*100 for p in prices], now_dt, 0, {})
    assert r1 == r2

@pytest.mark.asyncio
async def test_assert_cheaper_hours_tomorrow_not_lower_offset_than_today():
    ...
    offset_dict = await set_offset_dict(prices + prices_tomorrow, now_dt, 0, {})
...
@pytest.mark.asyncio
async def test_smooth_transistions_no_weather_prog_nothing_exceeds_tolerance():
    _tolerance = 3
    indoors_preset = HvacPresets.Normal
    prices = P231213
...
@pytest.mark.asyncio
async def test_adjust_to_treshold_no_exceeding_values():
    _tolerance = 3
    indoors_preset = HvacPresets.Normal
    prices = P231213
    prices_tomorrow = P231214
